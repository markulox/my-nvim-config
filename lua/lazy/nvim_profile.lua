...
function get_profile_name()
    local f = io.open(get_first_open_dir() .. "/.nvim_profile", "r")
    if f then
...
vim.api.nvim_create_user_command("ProfileSet", function(opts)
    ...
    if vim.fn.isdirectory(profile_path) == 1 then
        local f = io.open(get_first_open_dir() .. "/.nvim_profile", "w")
        if f then
            f:write(profile_name)
            f:close()
            vim.notify("[NVIM PROFILE]: Please restart nvim to apply changes")
        else
            vim.notify("[NVIM PROFILE]: Can't create .nvim_profile", vim.log.levels.ERROR)
        end
    else
        vim.notify(
